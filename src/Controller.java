    ...
    private ArrayList<Product> sortProduct(ArrayList<Product> list, int type) {
        for (int i = 0; i < list.size(); i++) {
            for (int j = 1; j < list.size() - 1; j++) {
                ...
                if ((type == 1 && list.get(j - 1).getPrice() > list.get(j).getPrice()) || (type == 2
                        && list.get(j - 1).getProductName().compareTo(list.get(j).getProductName()) > 0)) {
                    ...
                    if (loginID.charAt(0) == 'O') {
                        switch (option) {
                            ...
                            case 3:
                                ...
                                break label;
                            case 4:
                                displayShipment(list.get(index).getProductID());
                            default:
                                ui.displayMessage("invalid");
                        }
                    } else {
        ...
        for (Cart c : cartList) {
            ...
            for (Shipment s : shipmentList) {
                if (c.getProduct().getProductID() == s.getProductID() && s.getUnlockedQuantity() > 0) {
                    ...
                    expire = LocalDate.of(Integer.valueOf(date[0]), Integer.valueOf(date[1]), Integer.valueOf(date[2]))
                            .plusDays(add);
                    break;
                }
            }
                ...
                while (parser.hasNextLine()) {
                    ...
                    if (i == 0) {
                        ...
                        for (int a = 0; a < length - 2; a++)
                            items.add(temp[i]);
                        transactionList.add(new Transaction(items, Double.valueOf(temp[length - 3]), temp[length - 2],
                                temp[length - 1]));
                    } else if (i == 2) {
                        String temp[] = oneInfo.split(";");
